...
export function DataTable<TData, TValue>({
  ...
  // Convert all rows to a JSON array
const rowsJsonArray = table.getRowModel().rows.map(row => row.original);

// If you want to log or use it:
console.log(JSON.stringify(rowsJsonArray));
                      ...
                      <CollapsibleContent asChild className={
                        ...
                        <tr className="p-0 border-b">
                          <td colSpan={columns.length + 1} className="p-0 border-t-0">
                            <SubDataTable row={row} originalData={originalData} />
                          </td>
                        </tr>
                      </CollapsibleContent>
                    </>
